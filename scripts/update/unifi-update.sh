...
support_file() {
  ...
  if [[ -e "${eus_dir}" || -e "${unifi_db_eus_dir}" ]]; then
    find "${eus_dir}" "${unifi_db_eus_dir}" -type d -print -o -type f -print &> "${support_dir}/dirs_and_files"
  fi
  ...
  if [[ "${unifi_database_move_sucess}" == 'true' ]]; then
    ...
    if [[ "${revert_mongodb_upgrade_package_changes}" == 'true' && -n "${mongodb_org_upgrade_from_version_with_dots}" ]]; then
      ...
      if [[ "${glennr_compiled_mongod_purged_server_import}" == 'true' ]]; then
        ...
        if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_downgrade_option[@]}" "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' purge "${gr_mongod_name}" &>> "${eus_dir}/logs/mongodb_upgrade_${mongodb_upgrade_from_version::2}_to_${mongo_version_max}-downgrade-abort.log"; then
      ...
      while read -r mongodb_package; do
        ...
        if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_downgrade_option[@]}" "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "${mongodb_package}=${mongodb_org_upgrade_from_version_with_dots}" &>> "${eus_dir}/logs/mongodb_upgrade_${mongodb_upgrade_from_version::2}_to_${mongo_version_max}-downgrade-abort.log"; then
      ...
      if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_downgrade_option[@]}" "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "mongodb-org-server=${mongodb_org_upgrade_from_version_with_dots}" &>> "${eus_dir}/logs/mongodb_upgrade_${mongodb_upgrade_from_version::2}_to_${mongo_version_max}-downgrade-abort.log"; then
...
if "$(which dpkg)" -l unifi-core 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
  ...
  if command -v uos >/dev/null 2>&1 && uos runnable --help >/dev/null 2>&1; then unifi_core_runnable_system="true"; fi
...
repackage_deb_file_required_package() {
  if ! "$(which dpkg)" -l zstd 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
    ...
    if ! "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install zstd &>> "${eus_dir}/logs/required.log"; then
      ...
      if [[ "${repo_codename}" =~ (precise|trusty|utopic|vivid|wily|yakkety|zesty|artful|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish) ]]; then
        repo_codename_argument="-security"
        repo_component="main"
      elif [[ "${repo_codename}" =~ (jammy|kinetic|lunar|mantic|noble|oracular|plucky|questing|resolute) ]]; then
        repo_component="main"
      elif [[ "${repo_codename}" =~ (wheezy|jessie|stretch|buster|bullseye|bookworm|trixie|forky|unstable) ]]; then
        repo_component="main"
      fi
  ...
  if ! "$(which dpkg)" -l binutils 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
    ...
    if ! "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install binutils &>> "${eus_dir}/logs/required.log"; then
  ...
  if [[ "${mongodb_org_version_no_dots::1}" -ge "5" ]]; then
    if [[ -z "$(mongosh --version 2> /dev/null)" ]]; then
      while read -r mongosh_package; do
        ...
        if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' purge "${mongosh_package}" &>> "${eus_dir}/logs/mongosh-corruption.log"; then
    ...
    if ! apt-cache depends "${mongodb_org_server_package}" &> /dev/null; then silent_run_apt_get_update="true"; run_apt_get_update_with_sha1_fallback; fi
...
if "$(which dpkg)" -l mongodb-server 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
  if ! "$(which dpkg)" -l mongodb-clients 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
    ...
    if ! "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install mongodb-clients &>> "${eus_dir}/logs/unifi-easy-update-script-required.log"; then
...
prevent_unifi_upgrade() {
  if [[ "${unifi_core_runnable_system}" == 'true' ]]; then return 0; fi
...
custom_url_install() {
  ...
  if [[ "${unifi_core_system}" != 'true' ]]; then
    ...
    "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "${unifi_temp}" &>> "${eus_dir}/logs/unifi-update.log" 2>&1 &
    update_progress_pid="$!"
    monitor_update_progress_pid "${update_progress_pid}"
    wait "${update_progress_pid}"
    update_progress_exit_code="$?"
...
mongodb_upgrade() {
  ...
  if "$(which dpkg)" -l "${mongodb_server_package_name}" 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
    ...
    if [[ "${install_mongodb_org_tools}" == 'true' ]]; then
      if "$(which dpkg)" -l mongo-tools 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
        ...
        if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' purge "mongo-tools" &>> "${eus_dir}/logs/unifi-database-required.log"; then
  ...
  if "$(which dpkg)" -l mongodb-server 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
    ...
    if ! [[ "${mongodb_server_ver::2}" =~ (26|24) ]]; then
      if ! "$(which dpkg)" -l mongo-tools 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
        if "$(which dpkg)" -l mongodb-database-tools 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
          ...
          if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' purge "mongodb-database-tools" &>> "${eus_dir}/logs/unifi-database-required.log"; then
        ...
        if ! "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install mongo-tools &>> "${eus_dir}/logs/unifi-database-required.log"; then
    ...
    if ! "$(which dpkg)" -l mongodb-clients 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
      ...
      if ! "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install mongodb-clients &>> "${eus_dir}/logs/unifi-database-required.log"; then
        echo -e "${RED}#${RESET} Failed to install mongodb-clients...\\n"
          if [[ "${os_codename}" =~ (trusty|utopic|vivid|wily|yakkety|zesty|artful|qiana|rebecca|rafaela|rosa|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar|mantic|noble|oracular|plucky|questing|resolute|sarah|serena|sonya|sylvia|tara|tessa|tina|tricia) ]]; then
            repo_component="main universe"
            repo_codename="xenial"
      ...
      case "$yes_no" in
         ...
         [Yy]*)
            ...
            while read -r reverse_depends_package; do
              ...
              if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' remove "${reverse_depends_package}" &>> "${eus_dir}/logs/mongodb_upgrade_${mongodb_upgrade_from_version::2}_to_${mongo_version_max}.log"; then
  ...
  if [[ -s "/tmp/EUS/mongodb/purge_packages_list" ]]; then
    ...
    if [[ "${unifi_downloaded}" == "true" ]]; then
      ...
      if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' remove unifi &>> "${eus_dir}/logs/mongodb_upgrade_${mongodb_upgrade_from_version::2}_to_${mongo_version_max}.log"; then
    ...
    while read -r mongodb_package_to_upgrade; do
      ...
      if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' purge "${mongodb_package_to_upgrade}" &>> "${eus_dir}/logs/mongodb_upgrade_${mongodb_upgrade_from_version::2}_to_${mongo_version_max}.log"; then
  ...
  while read -r mongodb_package; do
    ...
    libssl_installation_check
    check_dpkg_lock
    if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_downgrade_option[@]}" "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "${mongodb_package}${install_mongodb_version_with_equality_sign}" &>> "${eus_dir}/logs/mongodb_upgrade_${mongodb_upgrade_from_version::2}_to_${mongo_version_max}.log"; then
      ...
      if tail -n20 "${eus_dir}/logs/mongodb_upgrade_${mongodb_upgrade_from_version::2}_to_${mongo_version_max}.log" | grep -iq "uses unknown compression for member .*zst"; then
        ...
        check_dpkg_lock
        echo -e "${GRAY_R}#${RESET} ${mongodb_upgrade_mongodb_org_message} ${mongodb_package}..."
        if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_downgrade_option[@]}" "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "${repackage_deb_file_location}" &>> "${eus_dir}/logs/mongodb_upgrade_${mongodb_upgrade_from_version::2}_to_${mongo_version_max}.log"; then
          echo -e "${GREEN}#${RESET} Successfully ${mongodb_upgrade_mongodb_org_message_2} ${mongodb_package}! \\n"
        else
  ...
  if [[ "${mongo_version_max}" -ge '70' ]]; then
    if ! "$(which dpkg)" -l mongodb-mongosh-shared-openssl11 mongodb-mongosh-shared-openssl3 mongodb-mongosh mongosh 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi\\|^ri\\|^pi\\|^ui"; then
      ...
      if "${EUS_APT_ENV[@]}" env DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_downgrade_option[@]}" "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "${mongodb_mongosh_install_package_name}" &>> "${eus_dir}/logs/mongodb_upgrade_${mongodb_upgrade_from_version::2}_to_${mongo_version_max}.log"; then
...
if [[ "${mongodb_key_update}" == 'true' ]]; then run_apt_get_update_with_sha1_fallback; unset mongodb_key_update; get_mongodb_org_v; sleep 3; fi
...
if [[ "${perform_application_upgrade}" == 'true' && "${unifi_core_runnable_system}" != 'true' ]]; then java_install_check; fi
...
application_upgrade_releases() {
  ...
  unifi_autobackup_dir_check
  keystore_alias_check
  if [[ "${unifi_native_system}" == 'true' ]]; then unifi_package_name="unifi-native"; else unifi_package_name="unifi"; fi
  unifi_current="$("$(which dpkg)" -l "${unifi_package_name}" | tail -n1 |  awk '{print $3}' | cut -d'-' -f1)"
  ...
  if [[ "${unifi_core_runnable_system}" != 'true' ]]; then java_install_check; fi
  header
  check_service_timeoutsec_increase
  if [[ "${unifi_core_runnable_system}" != 'true' ]]; then
    check_service_overrides
    old_systemd_version_check
  ...
  unifi_deb_package_modification
  ignore_unifi_package_dependencies
  if version_ge "8.0" "$unifi_current" && version_ge "$application_version_release" "8.1"; then
  ...
  if [[ "${unifi_core_system}" != 'true' ]]; then
    ...
    if [[ "${unifi_core_runnable_system}" == 'true' ]]; then
      uos runnable install unifi --progress --version="${application_version_release}" &>> "${eus_dir}/logs/unifi-update.log" 2>&1 &
      update_progress_pid="$!"
      monitor_update_progress_pid "${update_progress_pid}"
      wait "${update_progress_pid}"
      update_progress_exit_code="$?"
...
run_upgrade_menu_for() {
  ...
  case "$app_key" in
    network)
      fallback_versions=(
        "5.6.40" "5.6.42" "6.5.55" "7.0.25" "7.1.68-124045abd4" "7.2.97-fa3c0ace6e"
